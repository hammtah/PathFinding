...
const App = () => {
    const startPlaceX = Math.floor(Math.random()*(COLS-2));
    const startPlaceY = Math.floor(Math.random()*(ROWS-2));
    const endPlaceX = Math.floor(Math.random()*(COLS-2));
    const endPlaceY = Math.floor(Math.random()*(ROWS-2));
    ...
    const playNodeSound = useCallback((kind) => {
        ...
        if (kind === 'visit') {
            ...
            osc.frequency.exponentialRampToValueAtTime(360, end);
            ...
            //final path visiting
             ...
             osc.frequency.exponentialRampToValueAtTime(810, end);
             gain.gain.setValueAtTime(0.0001, now);
             gain.gain.exponentialRampToValueAtTime(0.03, now + 0.01);
             gain.gain.exponentialRampToValueAtTime(0.0001, end);
        } else {
            //When encountering a swamp node(error sound)
            osc.type = 'sine';
        ...
        osc.start(now);
        osc.stop(kind === 'swamp' ? now + 0.12 : end);
    }, []);
        ...
        for(let i = 0; i < data.path.length; i++) {
            ...
            setTimeout(() => {
                setPath(prevState => [...prevState, data.path[i]]);
                                playNodeSound(swamps.some(s => s.x === data.path[i].x && s.y === data.path[i].y) ? 'swamp' : 'path');
